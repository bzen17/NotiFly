...
export async function getDashboardMetrics(user: any, range: string = '24h') {
  ...
  if (pg) {
    try {
      const tenantWhereSql = tenantClause;
      ...
      try {
        ...
        if (campaignIds.length > 0 && mongo) {
          try {
            ...
            logger.warn({ err }, 'Failed to load campaign meta');
          }
        }
        ...
        logger.warn({ err: e }, 'Error while processing per-rows for dashboard');
      }
      ...
      try {
        ...
        logger.warn({ err: e }, 'Failed to build dashboard time-series');
      }
    } catch (e) {
      totalDeliveries = 0;
      successCount = 0;
